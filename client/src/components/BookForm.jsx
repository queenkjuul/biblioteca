...
const BookForm = ({ headerString, canDelete, book }) => {
  ...
  useEffect(() => {
    // changed for simpler json-server interaction
    // if (book && book.Author) {
    //     setData({
    //         ...book,
    ...
    setCoverimg(
      book ? (book.coverimg ? book.coverimg : placeholder) : placeholder
    )
  }, [book])
  ...
  const cancelForm = (event) => {
    ...
    setModal({
      messageArray: [
        "Are you sure you want to cancel before adding this book?",
      ],
      isCritical: false,
      confirm: () => {
        window.history.back()
  ...
  const submitForm = (event) => {
    ...
    if (book && book.id) {
      ...
      axios
        .put(process.env.API_URL + "/api/books/" + book.id, data)
        .then(() => {
  ...
  const deleteBook = () => {
    axios
      .delete(process.env.API_URL + "/api/books/" + book.id)
      .then(() => {
        history.push("/bookshelf")
        location.reload()
      })
    ...
    <>
      ...
      <article className="book-form__container">
        <form className="book-form__card" onSubmit={submitForm}>
          <h1 className="book-form__header">
            {toTitleCase(headerString || "Header")}
          </h1>

          <label className="book-form__label--title" htmlFor="title">
            Title:
          </label>
          <input
          ...
          <label className="book-form__label--author " htmlFor="author">
            Author:
          </label>
          <input
          ...
          <button onClick={updateCover} className="book-form__addimg">
            Add Image
          </button>

          <label className="book-form__label--synopsis " htmlFor="synopsis">
            Synopsis:
          </label>
          <textarea
          ...
          <label
            className="book-form__label--publishDate"
            htmlFor="publishDate"
          >
            Published:
          </label>
          <div className="book-form__publishPage">
            ...
            />
            <label className="book-form__label--pageCount " htmlFor="pageCount">
              Pages:
            </label>
            <input
          ...
          <div className="book-form__controls">
            ...
            {canDelete ? (
              <button onClick={confirmDelete} className="btn--critical">
                DELETE BOOK
              </button>
            ) : (
              ""
